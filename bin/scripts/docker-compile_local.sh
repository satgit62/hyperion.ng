...
$DOCKER run \
	...
	-v "${SCRIPT_PATH}:/source:rw" \
	hyperionproject/hyperion-ci:$BUILD_TARGET \
	/bin/bash -c "mkdir -p / source/build-${BUILD_TARGET} && cd /source/build-${BUILD_TARGET} && 
	...
	#cp -v /source/build-${BUILD_TARGET}/bin/* /deploy/ 2>/dev/null || : &&
	#cp -v /source/build-${BUILD_TARGET}/Hyperion-* ./deploy/ 2>/dev/null || : &&
	exit 0;
	exit 1 " || { echo "---> Hyperion compilation failed! Abort"; exit 4; }

# overwrite file owner to current user
sudo chown -fR $(stat -c "%U:%G" $SCRIPT_PATH/build) $SCRIPT_PATH/build-${BUILD_TARGET}
